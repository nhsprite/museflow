...
- **JIT outline expansion**: `src/core/outline-expander.ts` generates a chapter outline and `ChapterPlan` immediately before drafting, using story arc progress, prior summaries, story state, pending tasks, and next-chapter boundary hints. Claimed mandatory/key beats are semantically verified against the outline description before drafting (`src/graph/services/plot-advance/beat-claim-verifier.ts`); unrealized claims are retried with a `beatClaimRejection` feedback section, then stripped after retry exhaustion (warning issue `outline.beat-claim-stripped`). Act-boundary pressure is injected at the outline stage as well (not only planning) in two tiers: pacing (medium) pressure fires when pending beats exceed the every-other-chapter pace (`pending * 2 > chapters left including current`), forcing a claim with the same rejection/retry loop but allowing a beatless soft landing with a warning issue (`outline.zero-claim-under-pacing-pressure`) after retry exhaustion; under high pressure (pending mandatory beats > chapters left after the current one) the outline prompt closes its zero-progress escape hatch (via `mandatoryBeatClaimRequired`), a zero-claim outline is rejected and retried with a `requiredClaims` feedback section, and on retry exhaustion the chapter aborts with a manual-resolution error instead of finalizing a beatless outline. The same high-pressure rule covers the claim-stripping path: if stripping unrealized claims would leave the outline with zero mandatory-beat claims, the chapter aborts as well rather than slipping through beatless.
- **Routing**: `src/core/chapter-generation/routing/` decides whether to draft, patch-fix, repair state, finalize, or stop for manual rewrite, using issue fingerprints for stall detection. When the same beat stays `beat_unproven` for consecutive rounds, routing treats it as an outline-level false claim: it revokes the claim from the persisted outline (`beat-claim-revocation.ts`) and discards the plan for rebuild instead of re-drafting forever; the beat stays pending for later chapters. Exception: under act-boundary high pressure (pending mandatory beats > chapters left in the act), revoking a _mandatory_ beat claim is blocked (revoking would finalize the chapter consuming nothing). Instead, routing feeds the prose-stage rejection back into outline generation (`outline-regeneration.ts`): the chapter outline is force-regenerated with a `BeatClaimPlanningRejection` (rejected claims + mandatory-claim requirement), keeping the claim while re-organizing the chapter around a provable event — capped at `MAX_OUTLINE_REGEN_ATTEMPTS = 2` per chapter; once exhausted, the chapter stops for manual resolution with a `mandatory_beat_unproven` blocking report. Key-beat revocation is unaffected.
- **Validation loop**: `validate_chapter_structured` verifies the structured event contract (evidence spans, foreshadow fulfillments, `STORY_FINAL_STATE` end-of-chapter declarations) before `validate_chapter_comprehensive` aggregates word-count, continuity, foreshadowing, and consistency checks. Final-state declarations that match the pre-chapter projection and appear among the chapter's own events are treated as "return to origin" and auto-complete the event stream (`source: 'final-state-completion'` events, recomputed every pass). Unauthorized (plan-external) `plot-advance` events are gated by the prose-level semantic verifier instead of hard rejection: verified ones are kept and consumed as real progress, unverified ones are dropped from `draftChapterEvents` with a warning (`structured.plot-advance-unauthorized-dropped`); other unauthorized event types remain hard errors. Because `STORY_EVENTS` is emitted before `CHAPTER_CONTENT`, an event's `@pN` evidence anchor is a prediction about not-yet-written paragraphs; when a claimed `plot-advance`/`foreshadow-fulfill` event fails semantic verification on its anchored paragraph, `src/graph/services/evidence-relocation.ts` searches the whole chapter (same strict criteria) for a paragraph that actually realizes the claim — found: the evidence pointer is corrected and the rejection dropped; not found: the rejection stands and reliably means the prose lacks the scene.
