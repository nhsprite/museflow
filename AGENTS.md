...
- **State repair**: when a rewrite loop's remaining errors are all state-corruption issues (stale state records contradicting finalized chapters), routing first enters the `repair_state` node (max 2 attempts per chapter): the LLM proposes structured state fixes that must pass strict structural validation (known entity ids, attribute enums, exact oldValue match, evidence chapter before the current one) before being written as canonicalFacts (`source: 'state_repair'`) and merged into storyState. Rejected proposals' reasons are stored in `session.stateRepairRejections` and fed back into the next attempt's prompt. The chapter is then re-validated; if repair attempts are exhausted, routing falls back to the manual `request_rewrite` path.
