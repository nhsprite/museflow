...
import { extractOutlineCharacters, mergeCharacterLists } from '../../src/utils/outline-characters.js'
import type { Character } from '../../src/types/character.js'

function makeOutline(number: number, title: string, description: string, introducedCharacters?: string[]) {
  return { id: `o${number}`, number, title, description, introducedCharacters }
}

function makeCharacter(name: string): Character {
  return { id: `c_${name}`, storyId: 's1', name, description: null, dialogueStyle: null, createdAt: 1 }
}
...
describe('mergeCharacterLists', () => {
  it('merges official and outline characters without duplicates', () => {
    ...
    const outline = [{ name: '陈裕堂', sourceChapter: 7, sourceTitle: '买办登场', sourceDescription: '陈裕堂登场。' }]
  ...
  it('skips outline characters already in official list', () => {
    ...
    const outline = [{ name: '陆廷槐', sourceChapter: 8, sourceTitle: '至亲反目', sourceDescription: '陆廷槐逼苏半城交权。' }]
    const merged = mergeCharacterLists(official, outline)
