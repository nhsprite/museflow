...
describe('runner revalidation', () => {
  ...
  it('re-adjudicates a blocking key beat against proven mandatory beats before blocking', async () => {
    const { runOneChapter } = await import('../../src/core/runner.js')
    const storyMemory = createEmptyStoryMemory()
    ...
    context.provider.chatStructured = vi.fn().mockResolvedValue({ covered: true })
    ...
    expect(updateLatestState).toHaveBeenCalledWith(
      expect.objectContaining({
        storyArc: expect.objectContaining({
          keyBeats: [expect.objectContaining({ id: 'A1-B1', coveredByMandatoryBeatId: 'A1-M1' })],
        }),
        pendingIssues: [],
      })
    )
  ...
  it('drops a stale required-beat retry issue carried from a previously blocked run', async () => {
    const { runOneChapter } = await import('../../src/core/runner.js')
    const staleIssue = {
      id: 'past-act-1-required-beat-A1-B1',
      ruleId: 'story-completion.required-beat-unproven',
    ...
    await runOneChapter('story-1', { mode: 'draft', retryIssues: [staleIssue] }, context)
  ...
  it('keeps blocking when re-adjudication rejects coverage for the blocking key beat', async () => {
    const { runOneChapter } = await import('../../src/core/runner.js')
    const storyMemory = createEmptyStoryMemory()
    storyMemory.beats['A1-M1'] = {
      id: 'A1-M1',
      description: '另一项已证明的义务',
      actIndex: 1,
      deadlineAct: 1,
      required: true,
      claimedIn: 2,
    ...
    const context = createMockContext()
    context.provider.chatStructured = vi.fn().mockResolvedValue({ covered: false })

    const result = await runOneChapter('story-1', { mode: 'draft' }, context)

    expect(context.provider.chatStructured).toHaveBeenCalledTimes(3)
    expect(mockGraph.invoke).not.toHaveBeenCalled()
    expect(result.rewriteRequested).toBe(true)
    expect(result.pendingIssues).toEqual(
      expect.arrayContaining([
