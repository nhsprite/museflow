...
// Older versions of updateLatestState generated custom IDs prefixed with "ckpt_",
// which LangGraph cannot use as a uuid5 namespace. Only repair that specific
// legacy pattern to avoid altering intentionally non-UUID test identifiers.
...
export class JsonCheckpointer extends BaseCheckpointSaver<string> {
  ...
  private loadCheckpointRecord(outputDir: string, checkpointId: string): CheckpointRecord | undefined {
    ...
    try {
      ...
      // Auto-repair checkpoint IDs generated by older code that used non-UUID
      // custom identifiers (e.g. "ckpt_..."). LangGraph uses checkpoint.id as a
      // namespace for uuid5, which requires a valid UUID-like string.
      const existingId = record.checkpoint.id as string
      if (isLegacyInvalidCheckpointId(existingId)) {
        const newId = randomUUID()
        logger.warn(`[MuseFlow] 检测到非法 checkpoint ID，自动修复: ${existingId} -> ${newId}`)
        record.checkpointId = newId
        record.checkpoint.id = newId
        const newPath = this.getCheckpointPath(outputDir, newId)
        writeFileAtomic(newPath, JSON.stringify(record, null, 2))
        unlinkSync(path)
        const latest = this.readLatestCheckpointId(outputDir)
        if (latest?.checkpointId === checkpointId || latest?.checkpointId === existingId) {
          this.writeLatestCheckpointId(outputDir, newId, record.checkpoint.ts)
        }
      }
      return record
    } catch {
