...
11. 适时埋下伏笔，为后续章节留下悬念
12. 【必须】每章字数要求：
    - 最低字数：不少于 3000 字（中文章符）
    - 理想字数：4000-6000 字
    - 严禁低于 2500 字，否则视为不合格
13. 【必须】章节结尾格式要求：
    - 必须以自然场景或人物动作收束，禁止使用"欲知后事如何，且听下回分解"、"（本章完）"、"第X章 XXX · 完"等任何形式的章节结束标记
    - 结尾不得使用总结性诗句、对联或说书人套语
    - 结尾应当是情节的自然停顿，留给读者余韵，而非刻意标注"结束"
14. 【必须】跨章节衔接要求：
    - 本章结尾的动作、对话或场景，不得与上一章结尾重复
    - 如果上一章以某个角色做某事结尾（如"写信"、"睡觉"、"醒来"），本章结尾不得再次出现相同角色做相同或高度相似的事情
    - 本章开头应当自然承接上一章的结尾，但不得简单重复上一章最后一段的内容
15. 【必须】时间线一致性：
    - 角色只能提及、回忆或期待他们已经经历或确认将要发生的事件
    - 严禁角色在叙述中提及尚未发生的未来事件作为已发生的回忆（除非是在预言、梦境等明确超现实场景中）
    - 涉及"前世记忆"、"命运预示"等内容时，必须使用"命中注定"、"将要"、"等待"等前瞻性措辞，不得使用"回忆"、"往事"等回顾性措辞
16. 以自然流畅的段落叙述为主
