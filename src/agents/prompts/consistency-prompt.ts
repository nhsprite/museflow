...
<content_to_check>
  ...
  <check_dimensions>
    ...
    <dimension name="outline" priority="critical">
      大纲合规检查：将当前章节正文与大纲（尤其是本章）进行逐条对照。
      <critical>核心事件偏离（必须报 error）：遗漏大纲明确列出的核心情节、改变核心动作的执行者、提前完成后续章节的核心结果、新增大纲未授权的新场景/新角色、关键台词未按大纲出现、人物出场顺序或行为与大纲核心冲突。</critical>
      <warning>执行细节差异（最多报 warning）：同一核心动作在具体操作方式、描写顺序、视角节奏上的合理差异，以及为叙事流畅而增加的简短过渡动作。只要不影响核心情节的功能结果、不导致读者困惑，就不得报 error。</warning>
    ...
    <dimension name="quality" priority="medium">
      ...
      - 人物刻画是否立体、对话是否生动
      - 是否存在 AI 惯用腔调或总结性套语
      - 是否存在大段复述既有信息而无新推进的注水篇幅：同一事实、线索清单或物品/状态清单在章内或跨章被反复重列，而不提供新的事件、信息或变化
      - 多角色同场且存在信息交流时，是否全章没有直接对白，人物交流被眼神、默契等非语言方式完全替代
      - 章节收束是否与前几章形成同一模板：相同的收尾动作、场景氛围或意象被连续重复使用
      - 正文是否混入元信息：引用作品自身章节号（如"第N章"）、内部结构化 ID、大纲/规划/节拍批注、协议标记（=== 区块）——此类属体例硬伤，报 error
      ${AI_PHRASE_PROHIBITIONS}
      质量问题只报 warning 或 info，除非严重到影响读者理解；元信息混入正文一律报 error。
    </dimension>
  </check_dimensions>
...
<severity_levels>
  <error>以下严重问题：跨章节的角色知识/对话矛盾、时间线严重矛盾、关键信息前后矛盾、因果关系完全断裂、必须回收的伏笔未回收、伏笔被提前剧透、伏笔回收方向矛盾、结构化状态矛盾、世界规则严重冲突、时代背景严重错误、大纲核心事件偏离。报 error 前必须同时满足：
  ...
  缺少以上任何一项，请降级为 warning 或 info。特别地，大纲执行细节差异不满足以上条件，不得报 error。</error>
  <warning>一般性不一致或中等质量问题：细节描述有轻微出入、时间标记不够明确、表述歧义、前面章节缺少铺垫但本章已补充说明、伏笔回收方式可以更好、角色对新信息的反应/联想存在多种解读可能、文笔略显平淡、节奏轻微失衡、个别 AI 腔调、大纲执行细节差异。</warning>
