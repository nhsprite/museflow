...
${state.chapterContent || '（无内容）'}
 ...
 一致性检测维度：
  ...
  5. **对话一致性（重点）**：角色说过的话是否前后矛盾
   ...
   - 例如：前一章角色亲口确认的信息，本章不能表现得像是第一次听说
 6. **信息一致**：关键信息（物品、消息、秘密）的传递和知悉情况是否前后一致
  7. **伏笔回收与保护（重点）**：
    - **必须回收的伏笔**：检查上述"必须在本章回收"和"已逾期"的伏笔是否在本章得到回收。如果未回收，报 error
    - **伏笔提前剧透**：检查本章是否提前泄露了尚未到期的伏笔内容（如某章埋伏笔暗示了一个悬念，却在远早于预期的章节就被完全揭示）
    - **伏笔回收一致性**：如果本章回收了某个伏笔，检查回收内容是否与埋下时的暗示方向一致（如伏笔暗示是"盟友"，回收却说是"路人"，则矛盾）
    - **正常伏笔**：检查"正常伏笔"是否被不当地提前揭示
    - **重要区分 - 本章新设情节 vs 伏笔**：本章首次引入的新情节、新场景、新描写（如本章末尾的环境预警、本章中首次出现的新势力）是本章的正常叙事推进，**不应**被视为"已埋伏笔的提前泄露"或"伏笔自指"。只有当本章揭示了**之前章节明确埋下**的悬念时，才构成伏笔回收。
  ...
  **判定标准**：
  ...
  【关于"大纲与剧情演进"的重要判定规则】
  - 故事大纲在不同章节可能会揭示新的地点、新的线索或修正之前的认知。**大纲层面的信息更新（如某个灵物的具体位置从模糊变为明确）是正常的情节推进，不应视为矛盾。**
  - 只有当**角色对已确立的事实表现出矛盾态度**（如角色已明确知道某信息，本章却表现得像第一次听说）时，才构成 consistency error。
  - 如果本章中某角色获得了新的信息（如通过占卜、感应、他人告知），并因此更新了认知，这是正常叙事推进，**不要报 error**。
  - **不要**因为"本章揭示了新的地点/线索，与前面章节中的模糊描述不同"而报 error。重点检查角色是否对新信息表现出不合理的矛盾态度。
